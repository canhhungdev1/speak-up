...
Dựa trên phân tích Usecase, dưới đây là thiết kế Cơ sở dữ liệu cho hệ thống bằng PostgreSQL (Sử dụng cho Supabase).

## 1. Sơ đồ thực thể kết nối (ER Diagram)

```mermaid
erDiagram
    users ||--o| profiles : "1-1 (Supabase Auth)"
    profiles ||--o{ user_progress : "has"
    profiles ||--o{ daily_checkins : "has"
...
Bảng này mở rộng từ bảng `auth.users` mặc định của Supabase (nơi quản lý email/password).
| Tên trường | Kiểu dữ liệu | Ràng buộc | Mô tả |
| :--- | :--- | :--- | :--- |
| `id` | `uuid` | PK, FK | Khóa chính, đồng thời là khóa ngoại liên kết 1-1 với `auth.users.id` của Supabase. |
| `full_name` | `varchar` | | Họ và tên hiển thị của người dùng. |
| `avatar_url` | `varchar` | | Đường dẫn (URL) tới ảnh đại diện của người dùng. |
| `role` | `enum` | Default: `USER`| Phân quyền hệ thống. Gồm 2 giá trị: `USER` (Học viên) và `ADMIN` (Quản trị viên). |
...
| `audio_url` | `varchar` | Not Null | Link trỏ tới file MP3 lưu trên Supabase Storage. |
...
| :--- | :--- | :--- | :--- |
| `id` | `uuid` | PK | Khóa chính. |
| `user_id` | `uuid` | FK | ID của học viên (`profiles.id`). |
...
| `checkin_date` | `date` | Not Null | Ngày thực tế học viên có tương tác (nghe audio) thuộc Set này. Mỗi ngày chỉ ghi 1 dòng (Unique constraint: `user_id`, `lesson_set_id`, `checkin_date`). Đếm số dòng này để xem đã đủ `required_days` chưa. |
