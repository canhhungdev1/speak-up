...
- **Tiến độ & Học sâu (Progress & Deep Learning Tracking):**
  ...
  - Yêu cầu check-in (đánh dấu đã học) tối thiểu 7 ngày cho một Lesson Set trước khi cho phép mở khóa (Unlock) Set tiếp theo. Trạng thái Unlock được cache lại để tăng tốc độ tải trang.
  - Thống kê (Dashboard): Streak (chuỗi ngày học liên tục được tính toán tự động mỗi ngày), tổng thời gian nghe trong tuần.

### 3.2. Dành cho Quản trị viên (Admin CMS)
- **Quản lý Khóa học & Bài học:** Thêm/Sửa/Xóa Course, Lesson Set.
- **Quản lý Người dùng & Bình luận:** Kiểm duyệt tương tác của học viên.
- **Upload Media:** Tải lên các file Audio (mp3) và đồng bộ với Storage.
...
- **Xác thực trực tiếp bằng Google OAuth:** Tối ưu hóa trải nghiệm đăng ký/đăng nhập chỉ với 1 click, loại bỏ hoàn toàn hệ thống mật khẩu truyền thống.
- **Thảo luận / Bình luận:** Khu vực comment dưới mỗi bài học để người dùng thảo luận, đặt câu hỏi và tương tác cộng đồng.
...
**Bước tiếp theo đã hoàn thành:**
1. **Usecase Diagram** (Biểu đồ luồng sử dụng).
2. **ERD (Entity-Relationship Diagram)** (Sơ đồ cơ sở dữ liệu PostgreSQL).
