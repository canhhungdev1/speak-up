...
1. Đọc tất cả file trong `.agent/learnings/`:
...
Mở và đọc file skill brainstorming tại đường dẫn tương đối trong dự án:
...
3. Viết code theo đúng conventions của project (xem `.agent/rules/general-coding-rules.md` và các file luật domain liên quan).
...
1. Kiểm tra build (tuỳ stack: `npm run build`, `ng build`, `./gradlew build`, `mvn clean compile`,...)
2. Lint check (tuỳ stack: `npm run lint`, `ng lint`, `eslint`, `./gradlew lint`,...)
3. Chạy unit test nếu có: `npm test`, `ng test`, `mvn test`, `./gradlew test`,...
